...
//        orderCardFirstLocator.shouldBe(visible);
    ...
    public void openOrderByNumberInLink(String orderNumber) {
        stepWithRole("Открыть заказ по номеру: " + orderNumber, () -> {
            orderCardsCollection.findBy(text(orderNumber))
                    .click();
//            orderNumberLinkCollection.findBy(partialText(orderNumber)).click();
//            orderCardTitleLocator.shouldBe(visible, Duration.ofSeconds(10)).shouldHave(partialText(orderNumber));
