...
//            qrCodeDescriptionLocator = driver.$$("div.mt-2.small").as("Коллекция Описание QR кодов");
    ...
    public void playSequence(Integer maxStep15) {
        stepWithRole("Проиграть гид: " + maxStep15 + 1 + " шагов", () -> {
            guideFrameLocator.shouldBe(visible, Duration.ofSeconds(10)); // added 10 seconds
            for (int i = 0; i < maxStep15; i++) {
                switch (i) {
                    case 0:
            ...
            System.out.println("guide played: " + " " + maxStep15 + 1 + " steps");
        });
    }
