...
public class NavPasswordTabProfilePageComponent extends BaseProfileSelfEmployedComponent {
    ...
    public void generatePassword() {
        stepWithRole("Сгенерировать пароль", () -> {
            ...
            stepWithRole("Убедиться, что сгенерирован Надежный  пароль: " + suggestedPasswordLocator.val(), () -> {
                suggestedPasswordLocator.shouldNotBe(Condition.empty).as("Предложенный пароль");
                System.out.println("suggestedPassword: " + suggestedPasswordLocator.val());
            });
            stepWithRole("Убедиться, что поле Новый пароль и Подтверждение пароля заполнены сгенерированным паролем: " + suggestedPasswordLocator.val(), () -> {
                ...
                stepWithRole("Убедиться, что в поле  Новый пароль и Повторите пароль отображается сгенерированный пароль: " + newPasswordInputLocator.val(), () -> {
