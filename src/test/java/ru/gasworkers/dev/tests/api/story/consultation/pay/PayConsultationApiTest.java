...
@Tag(AllureTag.API)
public class PayConsultationApiTest extends BaseApiTest {
    ...
    @EnumSource(PayConsultationCase.class)
    @Tag(AllureTag.POSITIVE)
    @DisplayName("Success case:")
    void positiveTestCase(PayConsultationCase testCase, @WithThroughUser(withOrderType = @WithOrderType(type = "consultation")) User client) {
        String token = loginApi.getTokenThrough(client);
        step("Get last order info", () -> {
            LastOrderInfoResponseDto draftLastOrderResponse = lastOrderInfoApi.getLastOrderInfo(token)
                    .statusCode(200)
                    .extract().as(LastOrderInfoResponseDto.class);
            orderId = draftLastOrderResponse.getData().getId();
        });
        ...
        step("Pick master", () -> {
            timetableId = selectConsultationMasterApi.selectMaster(testCase.getPickMasterDto(orderId), masterIdList.get(0), token)
                    .statusCode(200)
                    .extract().as(PickMasterResponseDto.class).getData().getTimetableId();
        });
        step("Apply master", () -> {
            receiptId = applyMasterApi.applyMaster(testCase.getApplyMasterDto(orderId, timetableId), masterIdList.get(0), token)
                    .statusCode(200)
